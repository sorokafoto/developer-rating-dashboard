...
|-----|------|-----------------|----------------|
| Медиана до первого **идентифицированного** звонка | `avg_call_response` | медиана минут до первого звонка по заявкам, где звонок был | `market.avg_response.mean`, `market.avg_response.best` |
| Заявки без идентифицированного звонка | `no_call_share` | % заявок без звонка за 72 ч | `market.no_callback_share.mean`, `.best` |
| Заявки с идентифицированным звонком | derived | `100 - no_call_share` | — |
| Касаний на отвеченную заявку | `avg_touches_per_responded_app` | среднее касаний на заявку с откликом | TBD (агрегат рынка) |
| Всего касаний | `total_touches` | сумма контактов за 72 ч | TBD |
| Макс. касаний на заявку | `max_touches_per_app` | рекорд по одной заявке | TBD |
| Проникновение SMS/мессенджеров | `messenger_penetration_share` | % заявок с SMS или мессенджером | `market.messengers.mean` |
| Отправлено заявок | `applications_sent` | факт N | план: 21 |
| Статус кворума | `insufficient_data` | true если N &lt; 11 | — |
...
| Средняя скорость ответа | `market.avg_response.mean` |
| Лучшая скорость | `market.avg_response.best` |
| Средняя доля без ответа | `market.no_callback_share.mean` |
| Лучшая доля ответа | `market.no_callback_share.best` |
...
Пример заголовка: «Как быстро перезванивают по заявке?» вместо «avg_call_response».
...
|-----------------|------------------|-------------------|
| **Медленный первый контакт** | `avg_call_response` &gt; `market.avg_response.mean` | amoCRM Enterprise, маршрутизация лидов, AI-секретарь Matvey |
| **Низкое покрытие заявок** | `no_call_share` &gt; `market.no_callback_share.mean` | amoCRM Enterprise, повышение дозваниваемости АТС, AI-секретарь |
| **Мало повторных касаний** | `avg_touches_per_responded_app` низкий vs типичный рынок | AI-секретарь, отдел реактивации, автоматизация в CRM |
